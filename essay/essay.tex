...
\newline %空一行
\newline
\textbf{关键字：信息学方法、遗传学问题、数学模型、二进制约定、生成函数、表现型映射、Trie 树、FWT 算法、时间复杂度、子集卷积。}
\newline %空一行
\end{abstract}
...
\begin{tikzpicture}
	\begin{semilogyaxis}[legend pos=outer north east]
		\addplot[domain=1:20,blue] {x * 2^x};
		\addlegendentry{$\mathcal O(k \times 2^k)$};
		\addplot[domain=1:20,yellow] {x^2 * 2^x};
		\addlegendentry{$\mathcal O(k^2 \times 2^k)$};
		\addplot[domain=1:20,red] {3^x};
		\addlegendentry{$\mathcal O(3^k)$};
		\addplot[domain=1:20,green] {x * 3^x};
		\addlegendentry{$\mathcal O(k \times 3^k)$};
		\addplot[domain=1:20,violet] {4^x};
		\addlegendentry{$\mathcal O(4^k)$};
	\end{semilogyaxis}
\end{tikzpicture}
\end{center}
