...
namespace Trie{
	...
	vector<int>inbreeding(int node,int depth){
		vector<int>v;
		v.resize(1<<(k-depth));
		if (depth==k){
			v[0]=tree[node].end;
		...
		for (int i=0;i<=2;++i){
			...
			vector<int>child=dfs(ch(node,i),depth+1);
			if (i==0){
				for (int j=0;j<child.size();++j) v[j]=(v[j]+4*child[j])%MOD;//四个子代全部都是隐性
			}
			else if (i==1){
				for (int j=0;j<child.size();++j) v[j]=(v[j]+child[j])%MOD,v[j+child.size()]=(v[j+child.size()]+3*child[j])%MOD;//熟悉的1:3比例
			}
			else if (i==2){
				for (int j=0;j<child.size();++j) v[j+child.size()]=(v[j+child.size()]+4*child[j])%MOD;
			}
		}
		return v;
	}
