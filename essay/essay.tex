...
% tikz 样式
\tikzset{
	bluenode/.style={circle,minimum width=30pt,minimum height=30pt,draw=blue},
	rednode/.style={circle,minimum width=30pt,minimum height=30pt,draw=red},
	blacknode/.style={circle,minimum width=30pt,minimum height=30pt,draw=black},
}
...
\begin{enumerate}
	...
	\item \textsl{转移函数} $\delta$ \qquad 转移状态是接受两个参数返回一个值的函数，第一个参数和返回值都是一个状态，第二个参数是一个字符。相当于 Trie 树节点之间的边（edge）。
\end{enumerate}
...
由字符集 $\{\texttt{a},\texttt{ab},\texttt{ac}\}$ 建立起的 Trie 树如图所示：

\begin{center}
\begin{tikzpicture}[sibling distance=80pt,bluenode/.style =]
	\node[bluenode] (1) at(0,0) {$start$};
	\node[blacknode] (2) at(-1.5,-2){};
	\node[blacknode] (3) at(1.5,-2){};
	\node[blacknode] (4) at(-1.5,-4){};
